...
const prospectsFile = path.join(dataDir, "prospects-status.json");
const prospectsSpreadsheetId = process.env.BC_PROSPECTS_SHEET_ID || "1bTTsZxlYgHhOISlAE-zQKuWPWFEgLNCkvFavWP26ALE";
const prospectsSheetGid = process.env.BC_PROSPECTS_SHEET_GID || "470599580";
...
const defaultDepartments = [
  "Dirección General, Gerencia y Administración",
  "Contabilidad",
  "Impuestos",
  "Pensiones y Seguridad Social",
  "Inmobiliaria",
  "Mercadeo, Marketing, Ventas y Comercial",
  "Gestión Humana y SG-SST",
  "Tecnología",
  "Facturación",
  "Temporales o varios"
];
...
async function loadDepartments(){
  ...
  if(fsSync.existsSync(departmentsFile)){
    const stored=JSON.parse(await fs.readFile(departmentsFile,"utf8"));
    if(Array.isArray(stored)&&stored.length)return stored;
  }
  const initial=defaultDepartments.map((name,index)=>({id:`DEP-${String(index+1).padStart(3,"0")}`,name,active:true,createdAt:new Date().toISOString()}));
  await fs.writeFile(departmentsFile,JSON.stringify(initial,null,2),"utf8");
  return initial;
}
let authUsers = await loadUsers();
let departmentDirectory = await loadDepartments();
const sessions = new Map();
const publicUser = (u) => ({ name:u.name, username:u.username, department:u.department, departmentGroup:departmentGroup(u.department), all:Boolean(u.all), active:u.active!==false });
...
async function persistOperationalData(){
  await Promise.all([
    fs.writeFile(userFile,JSON.stringify(authUsers,null,2),"utf8"),
    fs.writeFile(departmentsFile,JSON.stringify(departmentDirectory,null,2),"utf8"),
    fs.writeFile(dataFile,JSON.stringify(cases,null,2),"utf8"),
    fs.writeFile(tasksFile,JSON.stringify(tasks,null,2),"utf8"),
    fs.writeFile(messagesFile,JSON.stringify(messages.slice(0,1000),null,2),"utf8"),
    fs.writeFile(calendarFile,JSON.stringify(calendarItems,null,2),"utf8")
  ]);
}
function renameDepartmentReferences(previous,next){
  const matches=value=>sameDepartment(value,previous);
  authUsers=authUsers.map(item=>matches(item.department)?{...item,department:next}:item);
  cases=cases.map(item=>({...item,department:matches(item.department)?next:item.department,createdByDepartment:matches(item.createdByDepartment)?next:item.createdByDepartment,visibilityDepartments:Array.isArray(item.visibilityDepartments)?item.visibilityDepartments.map(value=>matches(value)?next:value):item.visibilityDepartments}));
  tasks=tasks.map(item=>matches(item.department)?{...item,department:next}:item);
  messages=messages.map(item=>({...item,fromDepartment:matches(item.fromDepartment)?next:item.fromDepartment,toDepartment:matches(item.toDepartment)?next:item.toDepartment}));
  calendarItems=calendarItems.map(item=>matches(item.department)?{...item,department:next}:item);
  sessions.forEach(session=>{const updated=authUsers.find(item=>item.username===session.user.username);if(updated)session.user=updated});
}
async function renameProfileReferences(previous,next){
  if(previous===next)return;
  cases=cases.map(item=>item.responsible===previous?{...item,responsible:next}:item);
  tasks=tasks.map(item=>item.responsible===previous?{...item,responsible:next}:item);
  await Promise.all([fs.writeFile(dataFile,JSON.stringify(cases,null,2),"utf8"),fs.writeFile(tasksFile,JSON.stringify(tasks,null,2),"utf8")]);
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/login-options" && req.method === "GET") return json(res,200,{users:authUsers.filter(item=>item.active!==false).map(publicUser),departments:departmentDirectory.filter(item=>item.active!==false).map(item=>item.name)});
    ...
    if (url.pathname === "/api/prospects" && req.method === "GET") { const user=sessionUser(req); if(!user)return json(res,401,{error:"Sesión no iniciada"}); try{const payload=await fetchProspects();const visible=user.all?payload.rows:payload.rows.filter(item=>sameDepartment(item.routingDepartment,user.department));return json(res,200,{spreadsheetId:prospectsSpreadsheetId,sheetName:"Prospectos",syncedAt:payload.syncedAt,prospects:visible});}catch(error){return json(res,502,{error:"No fue posible sincronizar la hoja de prospectos"})} }
    if (url.pathname === "/api/prospects/convert" && req.method === "POST") { const user=sessionUser(req); if(!user)return json(res,401,{error:"Sesión no iniciada"}); const input=await body(req);const prospectKeyValue=String(input?.prospectKey||"").trim();if(!prospectKeyValue)return json(res,400,{error:"El prospecto no tiene identificador"});let payload;try{payload=await fetchProspects()}catch{return json(res,502,{error:"No fue posible consultar el prospecto"})}const prospect=payload.rows.find(item=>item.key===prospectKeyValue);if(!prospect)return json(res,404,{error:"Prospecto no encontrado"});if(!user.all&&!sameDepartment(prospect.routingDepartment,user.department))return json(res,403,{error:"Este prospecto no corresponde a tu departamento"});const statuses=await readProspectStatuses();if(statuses[prospectKeyValue]?.convertedCaseId)return json(res,409,{error:"Este prospecto ya fue convertido en caso",caseId:statuses[prospectKeyValue].convertedCaseId});const requestedDepartment=String(input?.department||prospect.routingDepartment).trim();if(!user.all&&!sameDepartment(requestedDepartment,prospect.routingDepartment))return json(res,403,{error:"El prospecto debe conservar su departamento sugerido"});const responsible=String(input?.responsible||"").trim(),due=String(input?.due||"").slice(0,10);if(!responsible||!due)return json(res,400,{error:"El caso requiere responsable y fecha de vencimiento"});const item={id:`CRM-${Date.now().toString().slice(-6)}`,client:prospect.fullName||prospect.email||prospect.phone,department:requestedDepartment,responsible,service:prospect.service||"Solicitud recibida desde QR",status:"Recibido",due,alert:"Pendiente",phone:prospect.phone,email:prospect.email,channel:"QR",acquisitionSource:"Formulario QR",acquisitionDetail:[prospect.qrCode,prospect.siteId].filter(Boolean).join(" · ")||"BC Soluciones - Registro de prospectos QR",pending:"",notes:String(input?.notes||prospect.notes||"").trim(),observations:String(input?.notes||prospect.notes||"").trim()?[{by:user.name,at:new Date().toLocaleString("es-CO"),text:String(input?.notes||prospect.notes||"").trim()}]:[],prospectKey:prospect.key,prospectSource:"BC Soluciones - Registro de prospectos QR",createdBy:user.name,createdByUsername:user.username,createdByDepartment:user.department,visibilityDepartments:["Dirección General"]};cases.unshift(item);await fs.writeFile(dataFile,JSON.stringify(cases,null,2),"utf8");statuses[prospectKeyValue]={status:"Convertido",convertedCaseId:item.id,convertedAt:new Date().toISOString()};await fs.writeFile(prospectsFile,JSON.stringify(statuses,null,2),"utf8");await logEvent({type:"prospect_converted",username:user.username,department:item.department,prospectKey:prospect.key,caseId:item.id});return json(res,201,{case:item,prospect:{...prospect,status:"Convertido",convertedCaseId:item.id,convertedAt:statuses[prospectKeyValue].convertedAt}}); }
    ...
    if (url.pathname === "/api/calendar/resolution" && req.method === "POST") {
      ...
      const created=schedule.map(({days,date})=>({id:crypto.randomUUID(),type:days?"Tarea":"Evento",title:`${days?`Revisar en ${days} días`:"Vence hoy"} resolución DIAN ${number} · ${client}`.slice(0,180),description:`Verificar vigencia y rango de numeración autorizado; gestionar habilitación del rango no agotado o nueva autorización, según corresponda, para ${client} (NIT ${nit}). Resolución: ${number}. Vencimiento: ${expiry}.`,date,time:"",department:"Facturación",username:"",priority:days<=5?"Alta":"Media",status:"Pendiente",resolution:{client,nit,number,expiry,days,batch},createdBy:user.name,createdAt:now}));
    ...
    if (url.pathname === "/api/admin/directory" && req.method === "GET") { const user=sessionUser(req); if(!user||!user.all)return json(res,403,{error:"Solo Dirección General puede administrar perfiles y departamentos"}); return json(res,200,directoryPayload()); }
    if (url.pathname === "/api/admin/users" && req.method === "POST") {
      const user=sessionUser(req);if(!user||!user.all)return json(res,403,{error:"Solo Dirección General puede crear perfiles"});
      const input=await body(req),username=String(input?.username||"").trim().toLowerCase(),name=String(input?.name||"").trim(),department=String(input?.department||"").trim(),password=String(input?.password||"");
      if(!validUsername(username))return json(res,400,{error:"El usuario debe tener entre 3 y 50 caracteres: letras minúsculas, números, punto, guion o guion bajo"});
      if(name.length<3)return json(res,400,{error:"Ingresa el nombre visible del perfil"});
      if(!activeDepartment(department))return json(res,400,{error:"Selecciona un departamento activo"});
      if(password.length<10)return json(res,400,{error:"La contraseña temporal debe tener al menos 10 caracteres"});
      if(authUsers.some(item=>item.username.toLowerCase()===username))return json(res,409,{error:"Ya existe un perfil con ese usuario"});
      const created={username,name,department,all:false,active:true,passwordHash:crypto.scryptSync(password,username,32).toString("hex"),createdAt:new Date().toISOString(),createdBy:user.username};
      authUsers.push(created);await fs.writeFile(userFile,JSON.stringify(authUsers,null,2),"utf8");await logEvent({type:"profile_created",username,department,by:user.username});return json(res,201,publicUser(created));
    }
    if (url.pathname.startsWith("/api/admin/users/") && req.method === "PUT") {
      const user=sessionUser(req);if(!user||!user.all)return json(res,403,{error:"Solo Dirección General puede editar perfiles"});
      const username=decodeURIComponent(url.pathname.slice("/api/admin/users/".length)),target=authUsers.find(item=>item.username===username);if(!target)return json(res,404,{error:"Perfil no encontrado"});
      const input=await body(req),name=String(input?.name||"").trim(),department=String(input?.department||"").trim(),active=input?.active!==false;
      if(name.length<3)return json(res,400,{error:"Ingresa el nombre visible del perfil"});
      if(!target.all&&!activeDepartment(department))return json(res,400,{error:"Selecciona un departamento activo"});
      const replacement={...target,name,department:target.all?target.department:department,active:target.all?true:active,updatedAt:new Date().toISOString(),updatedBy:user.username};
      authUsers=authUsers.map(item=>item.username===username?replacement:item);await renameProfileReferences(target.name,replacement.name);await fs.writeFile(userFile,JSON.stringify(authUsers,null,2),"utf8");
      sessions.forEach((session,token)=>{if(session.user.username===username){if(replacement.active===false)sessions.delete(token);else session.user=replacement}});await logEvent({type:"profile_updated",username,department:replacement.department,by:user.username});return json(res,200,publicUser(replacement));
    }
    if (url.pathname === "/api/admin/departments" && req.method === "POST") {
      const user=sessionUser(req);if(!user||!user.all)return json(res,403,{error:"Solo Dirección General puede crear departamentos"});const input=await body(req),name=String(input?.name||"").trim();
      if(name.length<3)return json(res,400,{error:"El nombre del departamento debe tener al menos 3 caracteres"});if(departmentDirectory.some(item=>normalizedName(item.name)===normalizedName(name)))return json(res,409,{error:"Ese departamento ya existe"});
      const created={id:`DEP-${crypto.randomUUID().slice(0,8).toUpperCase()}`,name,active:true,createdAt:new Date().toISOString(),createdBy:user.username};departmentDirectory.push(created);await fs.writeFile(departmentsFile,JSON.stringify(departmentDirectory,null,2),"utf8");await logEvent({type:"department_created",department:name,by:user.username});return json(res,201,created);
    }
    if (url.pathname.startsWith("/api/admin/departments/") && req.method === "PUT") {
      const user=sessionUser(req);if(!user||!user.all)return json(res,403,{error:"Solo Dirección General puede editar departamentos"});const id=decodeURIComponent(url.pathname.slice("/api/admin/departments/".length)),target=departmentDirectory.find(item=>item.id===id);if(!target)return json(res,404,{error:"Departamento no encontrado"});
      const input=await body(req),name=String(input?.name||"").trim();if(name.length<3)return json(res,400,{error:"El nombre del departamento debe tener al menos 3 caracteres"});if(target.name==="Dirección General, Gerencia y Administración"&&name!==target.name)return json(res,400,{error:"El departamento principal de Dirección General conserva su nombre operativo"});if(departmentDirectory.some(item=>item.id!==id&&normalizedName(item.name)===normalizedName(name)))return json(res,409,{error:"Ese departamento ya existe"});
      const previous=target.name;departmentDirectory=departmentDirectory.map(item=>item.id===id?{...item,name,updatedAt:new Date().toISOString(),updatedBy:user.username}:item);if(previous!==name)renameDepartmentReferences(previous,name);await persistOperationalData();await logEvent({type:"department_updated",department:name,previousDepartment:previous,by:user.username});return json(res,200,directoryPayload());
    }
